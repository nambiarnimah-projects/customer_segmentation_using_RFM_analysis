...
Sales decimal(10,2));
select * from rfm1;
select count(distinct(CustomerID)) from rfm1;
show tables;

-- create a RFM table
select customerid,DATEDIFF((SELECT MAX(orderdate) FROM rfm1), MAX(orderdate)) AS recency,count(distinct(orderid)) as frequency,sum(sales) as monetary from rfm1 group by customerid order by recency,frequency,monetary;
-- create customer segments
select customerid,recency,frequency,monetary,6-ntile(5)over(order by recency) as rscore,ntile(5) over (order by frequency) as fscore,ntile(5) over(order by monetary) as mscore from 
(select customerid,DATEDIFF((SELECT MAX(orderdate) FROM rfm1), MAX(orderdate)) AS recency,count((orderid)) as frequency,sum(sales) as monetary from rfm1 group by customerid) as rfm1; 


-- generating rfm scores
select customerid,concat(rscore,fscore,mscore) as rfmscore from
(select customerid,recency,frequency,monetary,6-ntile(5)over(order by recency) as rscore,ntile(5) over (order by frequency) as fscore,ntile(5) over(order by monetary) as mscore from 
(select customerid,DATEDIFF((SELECT MAX(orderdate) FROM rfm1), MAX(orderdate)) AS recency,count(distinct(orderid)) as frequency,sum(sales) as monetary from rfm1 group by customerid) as rfmbase) as ntile1; 


-- catergorize the customers into groups
select customerid,rfmscore,
CASE
-- Highest value customers
WHEN rscore >= 4 AND fscore >= 4 AND mscore >= 4
THEN 'Best Customers'
-- Frequent buyers with good recency but not top spenders
WHEN rscore >= 3 AND fscore >= 4 AND mscore < 4
THEN 'Loyal Customers'
-- High spenders who don't buy very often
WHEN mscore >= 4 AND fscore < 4 AND rscore >= 3
THEN 'Big Spenders'
-- Very recent first/early purchases
WHEN rscore = 5 AND fscore = 1
THEN 'New Customers'
-- Haven't purchased recently but used to buy often
WHEN rscore <= 2 AND fscore >= 3
THEN 'At Risk'
-- Haven't purchased recently and rarely bought
...
(select customerid,recency,frequency,monetary,6-ntile(5)over(order by recency) as rscore,ntile(5) over (order by frequency) as fscore,ntile(5) over(order by monetary) as mscore from 
(select customerid,DATEDIFF((SELECT MAX(orderdate) FROM rfm1), MAX(orderdate)) AS recency,count(distinct(orderid)) as frequency,sum(sales) as monetary from rfm1 group by customerid) as rfmbase) as ntile1) as categories;
